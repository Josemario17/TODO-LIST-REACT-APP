...
function Task({text, TaskDone, SubKey, eliminar}){
    const [checked, setChecked] = useState(false)
    function CheckedListAdd(){
        checked ? setChecked(false) : setChecked(true)
        checked ? checked : TaskDone(text)
    }
    return (
        <div className="relative w-full h-12 my-2 bg-gray-300 rounded-sm px-2 flex items-center justify-between">
            <div className="w-11/12 bg flex items-center gap-2">
                ...
                <input type="text" value={text} disabled className={`w-full bg-transparent text-gray-800 outline-none ${checked ? 'line-through' : ''}`} placeholder="default value"/>
           </div>
           <div>
            <button onClick={e => eliminar(SubKey) } className="w-10 text-gray-800 h-full items-center">
